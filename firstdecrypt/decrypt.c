...
//  int EVP_CipherFinal(EVP_CIPHER_CTX *ctx, unsigned char *outm, int *outl);
    ...
    unsigned char key_ex[] = "0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF";
    unsigned char iv_ex[]  = "11111111111111112222222222222222";
    unsigned char ciphertext_base64[] = "jyS3NIBqen2CWpDI2jkSu+z93NkDbWkUMitg2Q==";

    unsigned char ciphertext_hex[]="8f24b734806a7a7d825a90c8da3912bbecfddcd9036d6914322b60d9";

    unsigned char key[strlen(key_ex)/2];
    for(int i = 0; i < strlen(key_ex)/2;i++){
        sscanf(&key_ex[2*i],"%2hhx", &key[i]);
    }
    
    unsigned char iv[strlen(iv_ex)/2];
    for(int i = 0; i < strlen(iv_ex)/2;i++){
        sscanf(&iv_ex[2*i],"%2hhx", &iv[i]);
    }
